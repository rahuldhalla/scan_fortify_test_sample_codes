...
int main()
{
    ...
    // Test 2: Null buffer
    result = snprintf(NULL, sizeof(buffer), "This should fail.");
    printf("Test 2: result=%d\n", result);

    // Test 3: Null format
    result = snprintf(buffer, sizeof(buffer), NULL);
    printf("Test 3: result=%d\n", result);

    // Test 4: Buffer size zero
    result = snprintf(buffer, 0, "This should also fail.");
    printf("Test 4: result=%d\n", result);

    // Test 5: Truncated output
    result = snprintf(buffer, 10, "This string is too long for the buffer.");
    printf("Test 5: result=%d, buffer='%s'\n", result, buffer);

    // Test 6: Buffer size exceeds RSIZE_MAX
    result = snprintf(buffer, RSIZE_MAX + 1, "Exceeds max size.");
